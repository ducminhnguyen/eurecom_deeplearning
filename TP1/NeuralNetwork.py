...
class NeuralNetwork(object):
    ...
    def predict(self, test_data):
        ...
        for testcase in test_data:
            ...
            prediction = self.feedForward( testcase[0] )
            prediction = np.argmax(prediction)
            count = (count + 1) if (answer - prediction) == 0 else count  
        return count
    
    def predictWithLabel(self, test_data):
        """ Evaluate performance by counting how many examples in test_data are correctly 
            evaluated. """
        count = 0.0
        predictVal = []
        for testcase in test_data:
            answer = np.argmax( testcase[1] )
            prediction = self.feedForward( testcase[0] )
            predictVal.append((prediction >= np.max(prediction)).astype(int))
            prediction = np.argmax(prediction)
            count = (count + 1) if (answer - prediction) == 0 else count  
        return (count, predictVal)
